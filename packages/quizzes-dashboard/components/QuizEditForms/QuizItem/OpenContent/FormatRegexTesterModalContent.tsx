...
export const FormatRegexTesterModalContent = ({ item }: ModalContentProps) => {
  ...
  const checkTestAnswer = (): boolean => {
    ...
    const test = variables.formatRegexTestAnswer
    const validatorRegex = variables.formatRegex
    const validator = new RegExp(validatorRegex.trim(), "i")
    const cleanedInput = test
      .replace(/\0/g, "")
    ...
    <>
      ...
      <ModalContent>
        <TextField
          ...
          variant="outlined"
          onChange={event =>
            dispatch(setFormatRegexTestAnswer(storeItem.id, event.target.value))
          }
        />
      </ModalContent>
      <ModalContent>
          ...
          {variables.regex ? (
            <>
              {variables.validRegex ? (
                ...
                <>
                  <Typography>Invalid Regex</Typography>
                </>
              )}
            </>
          ) : (
            <>
              ...
              <Typography>Validity regex not provided</Typography>
            </>
          )}
        </ModalTextContainer>
      </ModalContent>
...
export default FormatRegexTesterModalContent
