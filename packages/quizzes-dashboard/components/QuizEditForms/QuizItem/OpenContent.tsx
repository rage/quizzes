...
const OpenContent = ({ item }: openContentProps) => {
  ...
  const [regex, setRegex] = useState(storeItem.validityRegex)
  const [validRegex, setValidRegex] = useState(false)

  useEffect(() => handleRegexChange(regex ?? ""), [regex])
  ...
  const checkTestAnswer = (): boolean => {
    ...
    const test = testAnswer ?? ""
    const validatorRegex = regex ?? ""
    ...
    <>
      ...
      <RegexContainer>
        <TextField
          ...
          variant="outlined"
          value={regex}
          helperText={!validRegex && "Invalid regex"}
          onChange={event => {
      ...
      <StyledModal open={test} onClose={() => setTest(false)}>
        <StyledBox>
          <CloseButton onClick={() => setTest(false)} size="small">
                    ...
                    <>
                      {checkTestAnswer() ? (
                        ...
                        <>
                          ...
                          <Typography>
                            Given test doesn't match regex
                          </Typography>
                        </>
                      )}
                    </>
